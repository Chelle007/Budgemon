...
export default function EditTransactionPage() {
  ...
  const { user, loading, cards, transactions, handleTransactionUpdate, handleTransactionDelete, fetchTransactions, darkMode } = useUser();
    ...
    return (
      <div className={`font-sans max-w-md mx-auto h-screen flex items-center justify-center ${isDark ? 'bg-black' : 'bg-gray-100'}`}>
        <div className="text-center px-6">
          <p className={isDark ? 'text-gray-300 mb-4' : 'text-gray-600 mb-4'}>{error || 'Transaction not found.'}</p>
          <button
            onClick={() => router.push('/app?tab=dashboard')}
            className={`px-4 py-2 text-white rounded-lg transition ${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-900 hover:bg-gray-800'}`}
          >
            Back to Dashboard
          </button>
        </div>
    ...
    <div className={`font-sans max-w-md mx-auto h-screen shadow-2xl overflow-hidden ${isDark ? 'bg-black' : 'bg-gray-100'}`}>
      <div className={`flex flex-col h-full ${isDark ? 'bg-black' : 'bg-white'}`}>
        <div className={`flex items-center justify-between p-4 border-b sticky top-0 z-10 ${isDark ? 'bg-black border-gray-800' : 'bg-white border-gray-100'}`}>
          ...
          <h2 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>Edit Transaction</h2>
          <button
            onClick={handleDelete}
            disabled={isDeleting}
            className={`p-2 rounded-full border transition disabled:opacity-50 ${isDark ? 'border-red-800 hover:bg-red-900/50 text-red-400' : 'border-red-200 hover:bg-red-50 text-red-600'}`}
